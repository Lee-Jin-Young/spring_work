	...
	<script>
		...
		document.querySelector(".insert-form")
			.addEventListener("submit", function(e) {
				if(!isLogin) {
					e.preventDefault();
					location.href="${pageContext.request.contextPath}/users/loginform?url=${pageContext.request.contextPath}/cafe/detail?num=${dto.num}";
				}
			}); //eventListener
		
		// 페이지 로딩 시점에 만들어진 1 페이지에 해당하는 댓글에 이벤트 리스너 등록
		...
		*/
		window.addEventListener("scroll", function() {
			...
			if(isBottom && !isLoading && !isLast) {
				...
				*/
				ajaxPromise("ajax_comment_list","get", "pageNum="+currentPage+"&num=${dto.num}")
				.then(function(response) {
					//html문자열을 응답받기 때문에 response.text()이용
				...
				.then(function(data) {
					...
					//새로 추가된 댓글 li 요소 안에 있는 댓글 수정폼에 이벤트 리스너 등록
			...
			for(let i=0; i<replyLinks.length; i++) {
				replyLinks[i].addEventListener("click", function() {
					...
					if(!isLogin) {
						...
						if(isMove) {
							location.href = "${pageContext.request.contextPath}/users/login_form?url=${pageContext.request.contextPath}/cafe/detail?num=${dto.num}";
						}
						return;
					}//isLogin
